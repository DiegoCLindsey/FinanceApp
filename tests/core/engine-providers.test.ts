  ...
  const loans: LoanItem[] = [
    {
      ...
      comisionAmort: 0.5,
      amortizaciones: [{ fecha: '2026-04-10', cantidad: 5000, tipo: 'plazo', simulacion: true }],
      cuenta: 'default',
      tags: ['hipoteca'],
    },
    {
...
describe('paridad provider de intereses', () => {
  const accounts: InterestAccount[] = [
    ...
    },
    {
      ...
      activo: true,
      interes: 2.2,
      periodoCobro: 'semanal',
      saldoInicial: 5000,
      fechaInicialSaldo: '2025-12-01',
      historicoSaldos: [{ fecha: '2026-02-01', saldo: 7000 }],
    },
  ...
  it('proyectarInteresesCuentas idéntico (vía generarExtracto: la función legacy es interna)', () => {
    ...
    const legacy = FM.generarExtracto([], [], accounts, config, null, []).map(
      ({ delta: _d, saldoAcum: _s, ...ev }: Record<string, unknown>) => ev,
    );
    ...
    const legacyConMovs = FM.generarExtracto(
      [],
      movimientos.map((m) => ({
        _id: m.sourceId,
        activo: true,
...
describe('paridad provider de aportaciones', () => {
  const accounts: ContributionAccount[] = [
    {
      ...
      planAportaciones: [{ _id: 'p1', importe: 300, periodicidad: 'mensual', fechaInicio: '2026-01-20', cuentaOrigen: 'default' }],
    },
    {
      _id: 'f2',
      nombre: 'Plan',
      ...
      modeloFondo: 'pension',
      planAportaciones: [
        { _id: 'p2', importe: 500, periodicidad: 'trimestral', fechaInicio: '2025-11-30', fechaFin: '2026-12-31', cuentaOrigen: 'acc2' },
      ],
    },
  ...
  it('proyectarAportaciones idéntico', () => {
    ...
    expect(proyectarAportaciones(accounts, range, ['f2', 'acc2'])).toEqual(
      FM.proyectarAportaciones(accounts, range.start, range.end, ['f2', 'acc2']),
    );
  });
});
