...
¿Continuar?`))return;const{aplicadas:n,errores:i}=a.adjuster.aplicarTodas(o,{hoy:a.hoy()});q(i.length>0?`${n.length} ajustadas, ${i.length} con error`:`${n.length} estimaciones ajustadas`,i.length>0?"warn":"ok"),a.onDatosCambiados(),e()})}const pn="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 14H4V8h16v10zM6 10h5v2H6v-2zm0 4h8v2H6v-2z";function mn(t){const a={cuentaId:"",mes:(t.hoy??J)().slice(0,7),filtroTexto:""},e=()=>{var r;return(r=t.onDatosCambiados)==null?void 0:r.call(t)},o=t.hoy??J,s={ledger:t.ledger,accounts:t.accounts,estimaciones:t.estimaciones,tagsConocidas:()=>t.tags.todas(),onDatosCambiados:e,hoy:o},n={precision:t.precision,adjuster:t.adjuster,estimaciones:t.estimaciones,onDatosCambiados:e,hoy:o};function i(r){const d=t.ledger.saldoTotal(o()),c=t.ledger.ultimaFecha(),x=t.ledger.transacciones().length;r.innerHTML=`
      ...
      </div>`}function x(p){const u=o(),v=a.get("config").usarInflacion||!1,b=[...u].sort((y,S)=>S.year-y.year),$=J(),A=new Date().getFullYear(),m=V(new Date(A+5,0,1)),g=V(new Date(A+10,0,1)),h=v&&u.length>0?pt(u,$,m):null,I=v&&u.length>0?pt(u,$,g):null;p.innerHTML=`
      <div class="page-header">
    ...
    <div class="flex gap-8 items-center" style="flex-wrap:wrap;row-gap:6px">
      ...
      </div>`}function u(m){const g=(m==null?void 0:m.tipo)==="transferencia",h=t.store.get("escenarios"),I=(m==null?void 0:m.escenarioIds)||[],f=(y,S,M,C,j="")=>`<div class="form-group"><label class="form-label">${l(S)}</label>
      ...
      <div class="grid-3 mt-8">
        ...
        <div class="form-group"><label class="form-label">Tipo frecuencia</label>
          <select class="form-select" id="ef-tipo-frec">
            ${Cn.map(([y,S])=>`<option value="${y}"${((m==null?void 0:m.tipoFrecuencia)??"mensual")===y?" selected":""}>${l(S)}</option>`).join("")}
          </select>
        </div>
      </div>
   ...
   ${o?`<span class="text-sm" style="margin-left:6px">${l(o)}</span>`:""}`;function ae(t,a,e){return t.length===0?"":`<div class="form-group mt-8"><label class="form-label">Supuestos</label>
    ...
    <div class="flex gap-8 mt-16" style="justify-content:flex-end">
      ...
      </div>`);f&&(D(f,"[data-opt-volver]",c),D(f,"[data-cancelar]",i))}function b(h){const I=p(h);r()&&q("Plan anterior eliminado, recalculando…");const{loans:f,expenses:y,accounts:S,config:M,nominas:C}=u(),j=d(()=>Te(f,y,S,M,{frecuencia:I.frecuencia,mesesHorizonte:I.horizonte,minAmortizable:I.minAmortizable,tipoAmort:I.tipoAmort,fechaPrimeraAmort:I.fechaPrimeraAmort,loanIds:I.loanIds,nominas:C,sourceAccountId:I.sourceAccountId,selectedMarginIds:I.selectedMarginIds}));if(!j)return;if(j.plan.length===0){v(`No hay excedente suficiente respetando los ${j.margenesAplicados} márgenes de seguridad activos en los próximos ${I.horizonte} meses para generar amortizaciones por encima del mínimo de ${z(I.minAmortizable)}.`,"Prueba a revisar los márgenes de seguridad, reducir el mínimo de amortización, o ampliar el horizonte.");return}e={plan:j.plan,tipoAmort:I.tipoAmort};const E=`✨ Plan de optimización · ${I.frecuencia===1?"Mensual":`Cada ${I.frecuencia} meses`} · ${I.horizonte}m`,F=n(E,`
      ...
      <div class="flex gap-8 mt-16" style="justify-content:flex-end">
        <button class="btn-secondary" data-opt-volver>← Cambiar parámetros</button>
        <button class="btn-secondary" data-cancelar>Cerrar</button>
      </div>`);T&&(D(T,"[data-opt-volver]",c),D(T,"[data-cancelar]",i),D(T,"[data-opt-usar]",R=>A(Number(R.getAttribute("data-opt-usar")))))}function A(h){var f;const I=a==null?void 0:a.resultados.find(y=>y.frecuencia===h);I&&(r(),m(I.plan,((f=I.plan[0])==null?void 0:f.tipoAmort)||"plazo",{titulo:`✨ Plan ${I.label} · aplicado`,resumen:I,fechaObjetivo:a==null?void 0:a.fechaObjetivo}))}function m(h,I,f){if(h.length===0)return;const y=new Map;for(const M of h){const C=y.get(M.loanId)??[];C.push({_id:`${io}${M.mes}_${M.loanId}`,fecha:M.fechaAmort,cantidad:M.cantidadAmort,tipo:I,simulacion:!0}),y.set(M.loanId,C)}let S=0;for(const M of t.loans()){const C=y.get(M._id);if(!C)continue;const j=(M.amortizaciones||[]).filter(E=>!ro(E._id));t.guardarAmortizaciones(M._id,[...j,...C]),S+=1}q(`Plan aplicado: ${h.length} amortizaciones en ${S} préstamo${S!==1?"s":""} (simulación)`),f?g(f):i(),t.refrescar([...y.keys()])}function g({titulo:h,resumen:I,fechaObjetivo:f}){const y=n(h,`
      <div class="grid-4 mb-14" style="gap:10px">
      ...
      <div style="max-height:260px;overflow-y:auto">
        <table class="table-wrap" style="width:100%">
          ...
          <tbody>${I.plan.map(S=>lo(S,!1)).join("")}</tbody>
        </table>
      </div>
    ...
    <div class="grid-4" style="gap:8px;font-size:12px">
  ...
  </div>`}const Nn="M11.8 10.9c-2.27-.59-3-1.2-3-2.15 0-1.09 1.01-1.85 2.7-1.85 1.78 0 2.44.85 2.5 2.1h2.21c-.07-1.72-1.12-3.3-3.21-3.81V3h-3v2.16c-1.94.42-3.5 1.68-3.5 3.61 0 2.31 1.91 3.46 4.7 4.13 2.5.6 3 1.48 3 2.41 0 .69-.49 1.79-2.7 1.79-2.06 0-2.87-.92-2.98-2.1h-2.2c.12 2.19 1.76 3.42 3.68 3.83V21h3v-2.15c1.95-.37 3.5-1.5 3.5-3.55 0-2.84-2.43-3.81-4.7-4.4z";function qn(t){const a=t.hoy??J;let e=!1;const o=new Set;let s=null;const n=()=>{var f;return(f=t.onDatosCambiados)==null?void 0:f.call(t)},i=()=>t.store.get("escenarios"),r=f=>{var y;return((y=i().find(S=>S._id===f))==null?void 0:y.nombre)??f};function d(f){if(!f.activo||f.simulacion)return!1;const y=tt(f).tabla.filter(S=>!S.esAmortizacion);return y.length===0?!0:y[y.length-1].fecha<a()}function c(f,y){const S=a(),M=S.slice(0,7),C=new Map;let j=0;for(const E of f){if(!E.activo||E.simulacion||y.has(E._id)||(E.fechaInicio||"")>S)continue;const F=tt(E).tabla.filter(P=>!P.esAmortizacion&&P.fecha.startsWith(M)),w=F.length>0?F[0].cuota:0;C.set(E._id,w),j+=w}return{porLoan:C,total:j,activos:[...C.values()].filter(E=>E>0).length}}function x(f){const y=t.store.get("config"),S=y.dashboardStart,M=y.dashboardEnd,C=Math.max(1,(G(M).getTime()-G(S).getTime())/(30.44*864e5));let j=0;for(const E of f)!E.activo||E.simulacion||(j+=tt(E).tabla.filter(F=>!F.esAmortizacion&&F.fecha>=S&&F.fecha<=M).reduce((F,w)=>F+w.cuota,0));return{media:j/C,desde:S,hasta:M}}function p(f){const y=[...t.store.get("loans")].sort((P,T)=>T.tin-P.tin),S=new Set(y.filter(d).map(P=>P._id)),M=e?y:y.filter(P=>!S.has(P._id)),C=c(y,S),j=x(y),E=t.store.get("config"),F=t.store.get("inflacion"),w=new Date(G(a())).toLocaleDateString("es-ES",{month:"long",year:"numeric"});f.innerHTML=`
      <div class="page-header">
      ...
      <div id="loans-list">
        ${M.length===0?'<div class="text-sm" style="text-align:center;padding:40px 0">Sin préstamos.</div>':M.map(P=>En(P,{periodos:F,usarInflacion:!!E.usarInflacion,hoy:a(),cuotaMes:C.porLoan.get(P._id)??0,completado:S.has(P._id),nombreEscenario:r})).join("")}
      </div>`;for(const P of f.querySelectorAll("[data-body-loan]"))o.has(P.dataset.bodyLoan??"")&&P.classList.add("open")}const u=()=>document.getElementById("modal-overlay"),v=()=>document.getElementById("modal-content"),b=()=>{var f;return(f=u())==null?void 0:f.classList.add("hidden")};function $(f,y){const S=u(),M=v();return!S||!M?null:(M.innerHTML=`<div class="modal-title">${l(f)}</div>${y}`,S.classList.remove("hidden"),D(M,"[data-cancelar]",b),M)}function A(f,y){const S=f?t.store.get("loans").find(C=>C._id===f)??null:null,M=$(f?"Editar préstamo":"Nuevo préstamo",Dn(S,t.store.get("accounts"),i(),a()));M&&(M.addEventListener("change",C=>{var j;(j=C.target)!=null&&j.matches("[data-dp-modo]")&&oo(M)}),D(M,"[data-guardar-loan]",C=>{m(M,C.getAttribute("data-guardar-loan")||"")&&(b(),y())}))}function m(f,y){const S=P=>{var T;return((T=f.querySelector(P))==null?void 0:T.value)??""},M=P=>{var T;return!!((T=f.querySelector(P))!=null&&T.checked)},C=S("#f-nombre").trim(),j=parseFloat(S("#f-capital")),E=parseFloat(S("#f-tin")),F=parseInt(S("#f-meses"),10);if(!C||!Number.isFinite(j)||!Number.isFinite(E)||!Number.isFinite(F))return q("Completa los campos obligatorios","err"),!1;const w={nombre:C,capital:j,tin:E,meses:F,fechaInicio:S("#f-fecha"),comisionApertura:parseFloat(S("#f-com-ap"))||0,comisionAmort:parseFloat(S("#f-com-am"))||0,diaPago:so(f),cuenta:S("#f-cuenta"),simulacion:M("#f-sim"),activo:M("#f-activo"),mostrarFechaFinEnDashboard:M("#f-mostrar-fin"),tipoTasa:S("#f-tipo-tasa"),basico:M("#f-basico"),tags:S("#f-tags").split(",").map(P=>P.trim()).filter(Boolean),escenarioIds:[...f.querySelectorAll(".loan-escenario:checked")].map(P=>P.value)};return y?(t.store.updateItem("loans",y,w),q("Préstamo actualizado")):(t.store.addItem("loans",{...w,amortizaciones:[]}),q("Préstamo creado")),n(),!0}function g(f,y,S){const M=t.store.get("loans").find(E=>E._id===f);if(!M)return;const C=y?(M.amortizaciones||[]).find(E=>E._id===y)??null:null,j=$(y?"Editar amortización":"Añadir amortización",Rn(f,C,i(),a()));j&&D(j,"[data-guardar-amort]",E=>{const[F,w]=(E.getAttribute("data-guardar-amort")||"").split("|");h(j,F,w)&&(b(),S([F]))})}function h(f,y,S){var T;const M=R=>{var N;return((N=f.querySelector(R))==null?void 0:N.value)??""},C=M("#am-fecha"),j=parseFloat(M("#am-cant"));if(!C||!Number.isFinite(j)||j<=0)return q("Fecha y cantidad requeridas","err"),!1;const E=t.store.get("loans").find(R=>R._id===y);if(!E)return!1;const F={fecha:C,cantidad:j,tipo:M("#am-tipo"),simulacion:!!((T=f.querySelector("#am-sim"))!=null&&T.checked),escenarioIds:[...f.querySelectorAll(".amort-escenario:checked")].map(R=>R.value)},w=E.amortizaciones||[],P=S?w.map(R=>R._id===S?{...R,...F}:R):[...w,{_id:Date.now().toString(36),...F}];return t.store.updateItem("loans",y,{amortizaciones:P}),q(S?"Amortización actualizada":"Amortización añadida"),n(),!0}function I(f,y,S){D(f,"[data-toggle-finalizados]",()=>{e=!e,y()}),D(f,"[data-nuevo-loan]",()=>A(null,y)),D(f,"[data-optimizar]",()=>S.abrir()),D(f,"[data-toggle-loan]",(M,C)=>{var w;if((w=C.target)!=null&&w.closest("button"))return;const j=M.getAttribute("data-toggle-loan"),E=[...f.querySelectorAll("[data-body-loan]")].find(P=>P.dataset.bodyLoan===j);(E==null?void 0:E.classList.toggle("open"))?o.add(j):o.delete(j)}),D(f,"[data-editar-loan]",M=>A(M.getAttribute("data-editar-loan"),y)),D(f,"[data-borrar-loan]",M=>{if(!X("¿Eliminar préstamo?"))return;const C=M.getAttribute("data-borrar-loan");t.store.removeItem("loans",C),o.delete(C),q("Eliminado"),n(),y()}),D(f,"[data-amort-loan]",M=>{const C=M.getAttribute("data-amort-loan");o.add(C),g(C,null,y)}),D(f,"[data-editar-amort]",M=>{const[C,j]=(M.getAttribute("data-editar-amort")||"").split("|");o.add(C),g(C,j,y)}),D(f,"[data-borrar-amort]",M=>{const[C,j]=(M.getAttribute("data-borrar-amort")||"").split("|"),E=t.store.get("loans").find(F=>F._id===C);E&&(t.store.updateItem("loans",C,{amortizaciones:(E.amortizaciones||[]).filter(F=>F._id!==j)}),q("Amortización eliminada"),n(),y([C]))})}return{id:"loans",route:"loans",nombre:"Préstamos",flagId:"loans",seccion:1,iconoPath:Nn,mount(f){const y=(S=[])=>{for(const M of S)o.add(M);p(f)};s??(s=On({loans:()=>t.store.get("loans"),expenses:()=>t.store.get("expenses"),accounts:()=>t.store.get("accounts"),nominas:()=>t.store.get("nominas"),config:()=>t.store.get("config"),guardarAmortizaciones:(S,M)=>{t.store.updateItem("loans",S,{amortizaciones:M}),n()},hoy:a,refrescar:y})),p(f),f.dataset.wired!=="1"&&(I(f,y,s),f.dataset.wired="1")}}}const Ln={transporte:125,restaurante:220,otros:null},kn={transporte:"Transporte",restaurante:"Restaurante",otros:"Otros"},Bn=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"],kt=(t,a,e,o,s="")=>`<div class="form-group"><label class="form-label">${l(a)}</label>
   ...
   <select class="form-select" id="${t}">
    ...
    </div>`}const n=A=>{var m;return((m=t.store.get("accounts").find(g=>g._id===(A||"default")))==null?void 0:m.nombre)??(A||"default")};function i(A,m,g){const h=m.reduce((y,S)=>y+(S.bruto||0),0),I=Qo(m,g),f=h>0?I/h*100:0;return`<div style="margin-bottom:16px">
    ...
    </div>`}function r(A){const m=o(),g=[...t.store.get("nominas")].sort((S,M)=>(M.bruto||0)-(S.bruto||0)),{grupos:h,sueltas:I}=Xo(g),f=t.store.get("accounts").filter(Qn),y=g.filter(S=>S.activo!==!1);A.innerHTML=`
      <div class="page-header">
  ...
  </div>`:""}function pi(t,a){const e=Dt(t,a.tramosGanancias);if(!e)return"";const o=a.config,s=a.flujos(t._id),n=G(o.dashboardStart),i=G(o.dashboardEnd),r=Math.max(0,(i.getTime()-n.getTime())/(30.44*864e5)),d=e.saldo+s.totalAportaciones-s.totalReembolsos,c=t.interes>0?Math.pow(1+t.interes/100,1/12)-1:0,x=d>0&&r>0?Math.max(0,d*Math.pow(1+c,r)):Math.max(0,d),p=e.costBase+s.totalAportaciones,u=Math.max(0,x-p),v=Ce(u,a.tramosGanancias),b=u>0?(v/u*100).toFixed(1):"0",$=t.interes>0?`${t.interes}% anual`:"sin rentabilidad",A=e.saldo>0?(e.plusvalia/e.saldo*100).toFixed(1):"0",m=(S,M,C)=>S.map(j=>`<div class="flex justify-between mt-4">
    ...
    <div class="flex gap-8 mt-12" style="justify-content:flex-end">
      ...
      <div class="flex gap-12 mt-8 flex-wrap" style="font-size:11px;color:var(--text3)">${M}</div>
...
Esto recalibra el punto de arranque del dashboard.`)){for(const R of w)t.store.updateItem("accounts",R._id,{saldoInicial:S(R)??R.saldoInicial??0,fechaInicialSaldo:P});q("Saldo base actualizado"),e(),F()}}function j(F,w,P){D(F,"[data-nueva-acc]",()=>y(null,w)),D(F,"[data-editar-acc]",T=>y(T.getAttribute("data-editar-acc"),w)),D(F,"[data-tramos-ganancias]",()=>P.abrir()),D(F,"[data-reset-base]",()=>C(w)),D(F,"[data-hist-acc]",T=>M(T.getAttribute("data-hist-acc"),w)),D(F,"[data-principal-acc]",T=>{const R=T.getAttribute("data-principal-acc");t.store.set("accounts",t.store.get("accounts").map(N=>({...N,esCuentaPrincipal:N._id===R}))),q("Cuenta marcada como principal"),e(),w()}),D(F,"[data-borrar-acc]",T=>{const R=T.getAttribute("data-borrar-acc");if(t.store.get("accounts").length<=1)return q("Debe existir al menos una cuenta","err");if(!X("¿Eliminar cuenta?"))return;t.store.removeItem("accounts",R);const _=t.store.get("accounts");_.length>0&&!_.some(k=>k.esCuentaPrincipal)&&t.store.set("accounts",_.map((k,L)=>L===0?{...k,esCuentaPrincipal:!0}:k)),q("Cuenta eliminada"),e(),w()}),D(F,"[data-inv-modo]",T=>{const[R,N]=(T.getAttribute("data-inv-modo")||"").split("|");s.set(R,N==="real"?"real":"proyeccion"),w()}),$.wire(F,w)}let E=null;return{id:"accounts",route:"accounts",nombre:"Cuentas y ahorro",flagId:"accounts",seccion:1,iconoPath:Ai,mount(F){const w=()=>m(F);E??(E=xi({store:t.store,onDatosCambiados:()=>{e(),w()},año:()=>Number(a().slice(0,4))})),m(F),F.dataset.wired!=="1"&&(j(F,w,E),F.dataset.wired="1")}}}const et=(t,a,e="var(--text)",o=!1)=>`<tr>
    ...
    ">${l(h)}</button>`}function $(g){const h=g.querySelector("#fisc-tabs"),I=g.querySelector("#fisc-tab-content");h&&(h.innerHTML=ho.map(([f,y])=>b(f,y)).join("")),I&&(I.innerHTML=v())}function A(g){g.innerHTML=`
      ...
      <table style="width:100%;font-size:13px;border-collapse:collapse">
        ...
        <tbody>${y.map(j=>{const{eventos:E}=r(j),F=j.fechaFin||S,w=Me(E,F),P=w!==null&&M!==null?w-M:null;return`<tr>
    ...
    </div>`}function u(){if(o){try{o.destroy()}catch{}o=null}}function v(y){const S=s(),M=r(null),C=[{label:"Base (sin supuesto)",color:"#6b7280",esBase:!0,puntos:Se(M.eventos,S.dashboardStart,S.dashboardEnd)}];return y.forEach((j,E)=>{const{eventos:F,horizonte:w}=r(j);C.push({label:j.nombre,color:j.color||_t[E%_t.length],puntos:Se(F,S.dashboardStart,w)})}),C}function b(y,S){u();const M=y.querySelector("#chart-comparacion");M&&(o=Pi(M,v(S)))}function $(y){u();const S=new Set(t.store.get("accounts").map(j=>j._id));for(const j of[...e])S.has(j)||e.delete(j);const M=n(),C=s().escenarioActivo||null;y.innerHTML=`
      <div class="page-header">
      ...
      ${M.length===0?`<div class="card mb-14" style="padding:20px 24px">
             ...
             </div>`:`<div>${M.map(j=>c(j,C)).join("")}</div>
             ...
             <div class="card mt-12" style="padding:14px" id="esc-comparativa">${x(M)}</div>`}`,M.length>0&&b(y,M)}const A=()=>document.getElementById("modal-overlay"),m=()=>document.getElementById("modal-content"),g=()=>{var y;return(y=A())==null?void 0:y.classList.add("hidden")};function h(y,S){const M=y?n().find(F=>F._id===y)??null:null,C=A(),j=m();if(!C||!j)return;const E=(M==null?void 0:M.color)||_t[0];j.innerHTML=`
    ...
    ${Ar(a)}
    ${Sr(t,a)}
    ${Mr(t,a,e)}`}function $r(t){if(t.avisos.length===0&&t.propuestas.length===0)return"";const a={error:"var(--red)",atencion:"var(--yellow)",info:"var(--text2)"},e=t.avisos.map(i=>`<div style="display:flex;gap:8px;font-size:12px;line-height:1.6;margin-bottom:5px">
  ...
  </div>`}function Sr(t,a){if(a.fases.length<=1)return"";const e=o=>{var s;return((s=t.objetivos.find(n=>n._id===o))==null?void 0:s.nombre)??o};return`<div class="card mb-14">
